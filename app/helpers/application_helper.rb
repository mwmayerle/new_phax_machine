module ApplicationHelper
	...
	def check_for_revoked_caller_id_numbers_and_unlinked_users
		...
		if unlinked_users.present?
			message = unlinked_users.map { |user| user.email }.join(", ").concat(" are not linked to a fax number and cannot fax. Click the 'Link/Unlink Users' button on your Dashboard to link these users to a fax number. ")
